...
@dlt.table(name="quarantine_sensor_readings",
...
@dlt.table(name="silver_production_runs",
...
def silver_production_runs():
    df = dlt.read_stream("bronze_production_runs")
...
@dlt.table(name="silver_maintenance_work_orders",
...
def silver_maintenance_work_orders():
    df = dlt.read_stream("bronze_maintenance_logs")
...
@dlt.table(name="silver_alarms", table_properties={"quality": "silver"},
...
def silver_alarms():
    df = dlt.read_stream("bronze_alarms_events")
...
@dlt.table(name="silver_quality_checks", table_properties={"quality": "silver"},
           comment="LIMS results with normalized line ids and spec-conformance flag.")
@dlt.expect_or_fail("valid_line", "line_id IS NOT NULL")
def silver_quality_checks():
    df = dlt.read_stream("bronze_quality_checks")
...
dlt.create_streaming_table("silver_dim_equipment",
    comment="SCD2 equipment dimension. History preserved across replacements/decommissions.")

dlt.create_auto_cdc_flow(
    target="silver_dim_equipment",
    source="bronze_equipment_registry",
    keys=["equipment_id"],
