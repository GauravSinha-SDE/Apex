...
│   │   └── 02f_gold_critical_work_orders.sql   # gold MV: critical WOs resolved, per technician/week
│   │       # All six read pipeline-created silver tables, so each must run AFTER the
...
against a real Databricks workspace (not just validated for syntax) — three times over: once
to get the platform itself deploying and running cleanly, once to actually ask Genie the
brief's 5 example questions against the deployed gold layer, and once more to close the one
gap (critical work orders) that a real mart could actually fix. All three rounds surfaced real
findings a code read wouldn't have caught.
...
5. **`sql_task.file` requires exactly one `CREATE MATERIALIZED VIEW`/`STREAMING TABLE` per file**
   ...
   (`02a`–`02f`). Running two in parallel hit a workspace-tier quota (max 1 concurrent
   DBSQL-type pipeline), so all six are sequenced, not parallel.
...
### Round 3 — closing the 5th example question

`gold.critical_work_orders` (per-technician, per-plant-local-week, `priority = 'Critical'` AND
`work_order_status = 'COMPLETED'`) — re-tested live against Genie afterward and confirmed
correct (e.g. T-Nguyen resolved 2 critical WOs the week of 2026-02-16). No new platform bug this
round; the only wrinkle was needing one more `completed_ts_local` column on
`silver.maintenance_work_orders` (same plant-local pattern as before) and a full-refresh to
backfill it.

Full account of all three rounds, including dead ends tried and abandoned (a notebook-based DDL
...
- `databricks.yml` — Asset Bundle: dev/stg/prod targets, `apex_pipeline` (Lakeflow Declarative
  Pipeline, serverless), `apex_bootstrap` job (schemas/ref DDL → seeds → one pipeline run → 6
...
- `src/pipelines/apex_pipeline.py` — schema-qualified table names; `silver.sensor_readings`'s
  ...
  added consistently across `production_runs`, `maintenance_work_orders` (both created and
  completed), `quality_checks`.
- `src/ddl/02a`–`02f` — six gold KPI marts, `USE CATALOG`-parameterized, `CREATE OR REPLACE`,
  sequenced after the pipeline and after each other. Covers 6 of the plant's 7 KPIs and 3 of the
  brief's 5 example questions — CIP Cycle Time has no source signal anywhere in the sample
  extracts (checked directly), so no mart for it.
